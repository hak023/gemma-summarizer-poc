...
sequenceDiagram
    ...
    Client->>IPC: 요청 전송
    rect rgb(240, 245, 255)
        Note over IPC,Postprocessor: SLM Agent
        IPC->>Gemma: 요약 요청
        Gemma->>IPC: JSON 응답
        IPC->>Postprocessor: 후처리 요청
...
sequenceDiagram
    ...
    rect rgb(240, 245, 255)
        Note over IPC,STTData: SLM Agent
...
sequenceDiagram
    ...
    rect rgb(240, 245, 255)
        Note over IPC,Fields: SLM Agent
...
- **SLM Agent**: 모든 내부 컴포넌트들
  - IPC 서버, 전처리기, 믿음2.0 모델, 후처리기, 로거
  - rect rgb(240, 245, 255) 구문으로 그룹핑하여 구분 표시
